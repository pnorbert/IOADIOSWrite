...
Description
    Convert ADIOS data to OpenFOAM data for restart
...
Usage
    ...
    \param -ascii \n
    Write data in ASCII format instead of binary.
...
int main(int argc, char *argv[])
{
    argList::addNote
    (
        "convert ADIOS data into OpenFOAM restart data"
    );
    ...
    if (binary && (sizeof(floatScalar) != 4 || sizeof(label) != 4))
    {
        FatalErrorIn(args.executable())
            ...
            << "Hence cannot use binary format. Please use -ascii"
            << exit(FatalError);
    }
...
#   include "createNamedMesh.H"
    ...
    // ADIOS/ directory in the case
    fileName fvPath(runTime.path()/"adiosData");
    ...
    if (isDir(fvPath))
    {
        ...
        {
            Info<< "Keeping old ADIOS files in " << fvPath << nl << endl;
        }
        else
        {
            Info<< "Deleting old ADIOS files in " << fvPath << nl << endl;
